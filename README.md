...
- Adds a compact fixed-position status chip with enabled/paused status, pruned count, manual prune control, and pause/resume control.
...
Reload the page normally to restore the full rendered conversation from ChatGPT. Edit `maxMessagesToKeep` in `content.js` to change how many newest rendered messages remain on the page.
...
ChatGPT's DOM changes frequently. Edit the centralized selector/config block at the top of `content.js` if pruning stops finding messages or becomes too aggressive. The script prefers conversation-turn containers, then falls back to article and role/message-id selectors.
